...
### 集成版适配记录(2026-09-14,最终部署 localhost:8081)

最终集成版(全部 14 任务)部署后 UI 结构变化,适配 5 处用例定位器,实跑 11/11 全绿(连续两轮,约 17s):

1. **路径1**:`/generate/images` 恢复为"图像工坊"目录页(不再重定向)——断言目录标题,
   点击第一个"使用此模型"进入 `/generate/image?modelId=N` 编辑器,填提示词后断言生成按钮可点。
2. **路径5**:`/generate/videos` 恢复为"视频工坊"目录页——断言标题与 SeedVR2 高清放大能力卡片
   (此前"高清处理无 UI 入口"的缺陷在集成版已消除,能力卡片正常展示)。
3. **路径3**:生产抽屉文案统一为"生产这一镜"(标题"{镜头} · 生产这一镜"、描述"会生成 3 个候选视频");
   行内生产按钮自带可访问名"生产镜头 N 的 3 个候选视频"(此前的可访问性缺陷已修复);
   抽屉打开前需先选中分集树第一行。
4. **路径4**:`/projects/{id}/source` 路由已移除,故事转剧本入口移至 `/projects/{id}/scripts`;
   弹窗表单为"文本模型"下拉 + "每块字符数" + "开始解析"(集成实况;任务群描述中的
   "创作模型/开始创作"与部署 DOM 不符,按实际 DOM 断言)。
5. **路径6**:分集合成入口在选中分集后才渲染——先点击分集树第一行再断言
   "合成本集视频/查看本集视频/重试合成/合成中"任一入口。

新增发现(集成版):
- 分镜行内生产按钮已带 `aria-label`("生产镜头 N 的 3 个候选视频")且常显,此前可访问性缺陷已修复;
- 视频工坊目录"已启用能力"与"待接入能力"两区存在同名条目(如 SeedVR2),断言需用 `.first()`,
  对用户而言两区语义易混淆,建议待接入条目加"未接入"角标(仅记录)。
---

## SW-T06 Round2 执行记录与发现(QA 子代理追加,2026-09-14,集成版回归)

### 执行方式
- 对集成部署(含 B1 回收站/B2 异常语义/字幕导出/自动分块 v2)全部真实 HTTP 实测 + Playwright E2E;
  未改产品代码;临时项目(id=9)已删除;未触碰他人并发任务。

### 回归汇总
1. 回收站全链路:13/13 PASS。建→软删→回收站可见→恢复(保留原 id)→项目内可见→再删→物理删除全通;
   uitest 恢复/彻底删除 zhangyz 资产均 403,回收站列表隔离(uitest total=0),分页生效。
   语义缺口:restore/purge 不存在 id → 500(R2-03)。
2. 异常语义矩阵:13/18。B2 已修复 Round1 的 403/404/400 ✓;新缺口 5 项:
   不存在项目 500(R2-03①)、回收站不存在 id 500×2(R2-03②③)、JSON 体型错误 500(R2-04)、
   HTTP 方法不支持 500(R2-05)。
3. 字幕导出:11/11 全绿(真实分集格式断言、secondsPerLine 越界 400/边界 1 与 60、空对白 400、越权 403、
   不存在 404)。Round1 三缺陷(abC/越权/不存在)全部确认修复。
4. 自动分块 v2:4000 字文本真跑 — chunkChars=999999→钳12000→1 集、=-1→钳2000→3 集,
   均真实 AI 转换且无丢字(PASS);chunkChars="abc"(JSON 体)→500(R2-04);
   默认模型路径挂起 >10 分钟无终态(R2-02)。
5. 170 集完整性:被并发 Agent 的重解析阻塞(其任务卡死 69/170,即 R2-02 证据);
   已验证已写 68 集为原文无丢字前缀且编号连续;Round 1 已对稳定数据集全量验证,建议解析终态后补全量。
6. E2E 回归:适配版(b627420)11/11 全绿(17.7s,对 8081 实跑)。

### 新缺陷(详见 BUGS.md)
- SW-T06-R2-01|P1 调度器 in(Iterable) 绑定错误,滞留同步每分钟失败(一行修复)
- SW-T06-R2-02|P2 默认模型挂起→解析永久卡死,滞留恢复无法自救,线程池可被占死
- SW-T06-R2-03|P3 B2 遗漏:不存在项目/回收站不存在 id 仍 500
- SW-T06-R2-04|P3 JSON 体类型错误 500 应 400
- SW-T06-R2-05|P4 方法不支持 500 应 405(含 ERROR 堆栈噪音)

### 需要决策(不空等)
- R2-02 的修复方向(读超时/看门狗/默认模型切换)需拍板;R2-01 一行修复建议尽快合入。
- 共享平台协作约定:并发解析会互相干扰(本轮 170 抽查与全量无丢字校验均被竞态影响),
  建议对脚本 5 这类共享数据建立"使用中"标记或每 Agent 用独立项目。
