    ...
    <div className="mx-auto w-full max-w-6xl space-y-8 px-6 py-6">
      ...
      <section aria-labelledby="video-models" className="space-y-4">
        ...
        {!loading && !error && models.length === 0 && (
          <p className="text-muted-foreground">暂无已启用的视频模型，请联系管理员完成配置。</p>
        )}
        {!error && (
            ...
            {models.map((model) => (
              <article key={model.id} className="flex flex-col gap-4 rounded-xl border border-border/30 bg-card p-5">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">{model.description || "时长、首尾帧与参考图要求将在生成编辑器中展示。"}</p>
                </div>
                <div className="mt-auto">
                  <Link className={buttonVariants({ variant: "outline" })} href={`/generate/video?modelId=${model.id}`}>使用此模型</Link>
                </div>
              </article>
            ))}
          </div>
